...
function detectServiceType() {
  ...
  if (cwd.includes('/src/services/business/') || cwd.includes('/services/business/')) {
    return 'business';
  } else if (cwd.includes('/src/services/inference/') || cwd.includes('/services/inference/')) {
    return 'inference';
  }
  ...
  constructor() {
    ...
    const logFormat = format.combine(
      ...
      format((info) => {
        ...
        const context = {
          service: detectedServiceType || process.env.SERVICE_NAME || 'playwllm',
    ...
    const consoleFormat = format.combine(
      ...
      format.printf(info => {
        ...
        const serviceType = info.context?.service ? `[${info.context.service}] ` : '';
    ...
    this.logger = createLogger({
      levels: LOG_LEVELS,
      level: process.env.LOG_LEVEL || (argv.env === 'production' ? 'info' : 'debug'),
      format: logFormat,
      transports: [
